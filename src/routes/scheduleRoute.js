...
router.get("/", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const scheduledMessages = await ScheduledMessageService.getAllByUser(
      userId
    );
...
router.put("/:id", authenticateToken, async (req, res) => {
  try {
    ...
    const updatedMessage = await ScheduledMessageService.updateById(
      id,
      userId,
      {
        message,
    ...
    if (!updatedMessage) {
      return res
        .status(404)
        .json({ message: "Mensaje programado no encontrado" });
    }
